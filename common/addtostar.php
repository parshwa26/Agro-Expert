...
if($key == "4336f0f199bd35b3275e7b77e6471da3985f963b")
{
	...
	$arr = array();
		...
		$check = mysqli_query($con,"select * from `starred` where lid = $lid AND qid = $qid AND status=1");
		if(!($chk = mysqli_fetch_array($check)))
		{
			...
			$insert = mysqli_query($con,"insert into `starred` (sid,lid,qid,status) values($no,$lid,$qid,1)");
			if($insert)
