...
if($key == "4336f0f199bd35b3275e7b77e6471da3985f963b")
{
	...
	//		echo "select * from expertaction where lid = $lid";
		if($chk){	
			...
			{
				$abcd = $check['rcid'];
				$a = mysqli_query($con,"SELECT * FROM question WHERE cid IN ($abcd) AND qid = $qid AND expertid = 0 ORDER BY qid DESC");

	//			echo "SELECT * FROM question WHERE cid = '".$check['cid']."' AND expertid = 0 ORDER BY qid DESC";
			...
			//			echo "SELECT $col as col,pid from category where cid='".$a1."'";
					...
					$arr[] = array('qid' => $row['qid'],'question' => $row['question'],'image' => $row['image'],'contentflag' => $row['contentflag'],'timestamp' => $row['timestamp'],'category' => $category,'phoneno' => $getphno['phoneno'],'status' => "success");	
